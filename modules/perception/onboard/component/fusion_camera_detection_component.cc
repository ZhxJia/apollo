...
// @description: load camera extrinsics from yaml file jac:加载相机外参
...
// @description: get project matrix jac:获取投影矩阵 内参
...
// jac!!20/1/7:初始化 目前入口
// jac!!20/1/7:读取proto各配置参数和相关路径信息
bool FusionCameraDetectionComponent::Init() {
  if (InitConfig() != cyber::SUCC) {
    AERROR << "InitConfig() failed.";
    return false;
  }
  //jac!!20/1/7:创建输出节点 采用模板类 具体语法先略过 内容是上述初始化文件中Fusion_camera_detection_component消息包中的消息
  writer_ =
  ...
  if (InitSensorInfo() != cyber::SUCC) {
    ...
    return false;
  } //jac!!20/1/7: 摄像头传感器相关信息的初始化 会打印传感器信息
  if (InitAlgorithmPlugin() != cyber::SUCC) {
    AERROR << "InitAlgorithmPlugin() failed.";
    return false;
  } //jac!!20/1/8: 相机相关算法（跟踪、检测等）插件的初始化
  if (InitCameraFrames() != cyber::SUCC) {
    AERROR << "InitCameraFrames() failed.";
    return false;
  } //jac!!20/1/13: 这部分的初始化内容可以参考camera/common/camera_frame.h -> 主要初始化了相机内外参　高度　data_provider
  if (InitProjectMatrix() != cyber::SUCC) {
    AERROR << "InitProjectMatrix() failed.";
    return false;
  }//jac!!20/1/14: 根据相机内外参获取投影矩阵和pitch_diff
  if (InitCameraListeners() != cyber::SUCC) {
    AERROR << "InitCameraListeners() failed.";
    return false;
  }//jac!!20/1/14: 设置监听节点，call_back_function:OnReceiveImage()
  if (InitMotionService() != cyber::SUCC) {
    AERROR << "InitMotionService() failed.";
    return false;
  }//jac!!20/1/14: 设置监听节点：call_back_function:OnMotionService() 车辆运动信息

  SetCameraHeightAndPitch();//jac!!20/1/14:　设置相机的高度和pitch角度？

  // Init visualizer 　　jac??20/1/14:单应性矩阵：－>透视变换 图像拼接
...
void FusionCameraDetectionComponent::OnReceiveImage(
  ...
  std::lock_guard<std::mutex> lock(mutex_); //jac!!20/2/6: 多进程访问同一资源 为保证数据一致性 采用mutex锁
  ...
  if (last_timestamp_ - msg_timestamp > ts_diff_) {
    ...
    return;
  } //jac!!/20/2/6: 收到的是之前的图像帧 略过 不进行进一步处理
  last_timestamp_ = msg_timestamp;
  ++seq_num_;

  // for e2e lantency statistics  端到端延迟统计
  {
  ...
  std::shared_ptr<apollo::perception::PerceptionObstacles> out_message(
      new (std::nothrow) apollo::perception::PerceptionObstacles); //jac!!20/2/6: std::nothrow作为new参数 当内存分配失败 返回NULL指针  
...
int FusionCameraDetectionComponent::InitConfig() {
  ...
  //jac!!20/1/7:配置文件路径：/onboard/proto/fusion_camera_detection_component.proto

  std::string camera_names_str = fusion_camera_detection_param.camera_names();//jac??: 这个camera_names 不知从哪定义的 
  ...
  //jac??20/1/7:input_camera与camera区别：input是实际输入？
  //jac!!20/1/7:boost::algorithm是字符串算法库
  ...
  camera_perception_init_options_.conf_file =
      fusion_camera_detection_param.camera_obstacle_perception_conf_file();//jac!!20/1/7:obstacle.pt
  ...
  //jac!!20/1/7:static_cast<type>(expression) 表达式强制类型转换
  ...
  std::string config_info_str =
      str(boost::format(format_str.c_str()) % camera_names_[0] %
          ...
          write_visual_img_);
  AINFO << config_info_str; //jac!!20/1/7:输出读取的消息配置文件初始化的信息
...
int FusionCameraDetectionComponent::InitSensorInfo() {
  ...
  auto *sensor_manager = common::SensorManager::Instance();//jac!!20/1/7:单例模式创建的SensorManger类,传感器管理相关函数。
  for (size_t i = 0; i < camera_names_.size(); ++i) {
    if (!sensor_manager->IsSensorExist(camera_names_[i])) {
      AERROR << ("sensor_name: " + camera_names_[i] + " not exists.");
      return cyber::FAIL;
    } //jac!!20/1/7:sensor_manager查看摄像头是否都存在

    base::SensorInfo sensor_info;//jac!!20/1/7:获取 摄像头传感器 名称 方向 类型 帧id
    if (!(sensor_manager->GetSensorInfo(camera_names_[i], &sensor_info))) {
      AERROR << "Failed to get sensor info, sensor name: " << camera_names_[i];
      return cyber::FAIL;
    }
    sensor_info_map_[camera_names_[i]] = sensor_info; //jac!!20/1/7:建立camera_name和Info的键值对

    std::string tf_camera_frame_id =
        sensor_manager->GetFrameId(camera_names_[i]);
    tf_camera_frame_id_map_[camera_names_[i]] = tf_camera_frame_id; //jac!!20/1/7:建立camera_name和帧id的键值对
    std::shared_ptr<TransformWrapper> trans_wrapper(new TransformWrapper);//jac!!jac??20/1/7:貌似进行了坐标变换等相关操作 二维到三维
    trans_wrapper->Init(tf_camera_frame_id);
    camera2world_trans_wrapper_map_[camera_names_[i]] = trans_wrapper;//jac!!jac??20/1/7:貌似建立了camera name和 trans_wrapper 这个类之间的键值对
  }

  // assume all camera have same image size jac!!20/1/7:所以这里只处理了camera 0
  base::BaseCameraModelPtr camera_model_ptr =
      sensor_manager->GetUndistortCameraModel(camera_names_[0]);//jac!!20/1/7:获得指向基础Camera类的指针
  image_width_ = static_cast<int>(camera_model_ptr->get_width());
  image_height_ = static_cast<int>(camera_model_ptr->get_height());

  //jac!!20/1/7:输出摄像头传感器的相关信息
  std::string format_str = R"(
      camera_names: %s %s
...
int FusionCameraDetectionComponent::InitAlgorithmPlugin() {
  camera_obstacle_pipeline_.reset(new camera::ObstacleCameraPerception); //jac!!20/1/8:reset是 unique_ptr获取该对象所有权
  if (!camera_obstacle_pipeline_->Init(camera_perception_init_options_)) { //jac!!20/1/8: camera::ObstacleCameraPerception 该类初始化传入配置文件proto参数
    AERROR << "camera_obstacle_pipeline_->Init() failed"; //jac!!20/1/13:初始化功能模块的相关参数：detector tracker transformer lane_detector calibration_service
    return cyber::FAIL;
  }
...
int FusionCameraDetectionComponent::InitCameraFrames() {
  ...
  // fixed size
  camera_frames_.resize(frame_capacity_); //jac!!20/1/13: 默认20
  ...
  for (const auto &camera_name : camera_names_) {
    ...
    data_provider_init_options.do_undistortion = enable_undistortion_;//jac!!20/1/13:图像去畸变
    ...
    data_provider->Init(data_provider_init_options); //jac??20/1/13:为数据分配内存
    data_providers_map_[camera_name] = data_provider;// jac!!20/1/13:camera_name与data_provider的map
  }
  ...
  for (const auto &camera_name : camera_names_) {
    ...
    LoadExtrinsics(FLAGS_obs_sensor_intrinsic_path + "/" + camera_name +
                       "_extrinsics.yaml",
                   &extrinsic); //jac??20/1/13:FLAGS_obs_sensor_intrinsic_path不清楚从哪加载进来的
  ...
  for (auto &frame : camera_frames_) {
    frame.track_feature_blob.reset(new base::Blob<float>()); //jac!!20/1/13:camera_frame的容量为20
...
int FusionCameraDetectionComponent::InitCameraListeners() {
  for (size_t i = 0; i < camera_names_.size(); ++i) {
    ...
    typedef std::shared_ptr<apollo::drivers::Image> ImageMsgType; //jac!!20/1/13 modules/drivers/proto/sensor_image.proto
    std::function<void(const ImageMsgType &)> camera_callback =
        std::bind(&FusionCameraDetectionComponent::OnReceiveImage, this,
                  std::placeholders::_1, camera_name);//jac!!20/1/14:回调函数, std::bind:绑定函数调用的某些参数 ,std::function 定义function类型　包含一个参数
    auto camera_reader = node_->CreateReader(channel_name, camera_callback);　//std::placeholders 表示一个占位符 与 OnReceiveImage的第一个参数对应
  }
  return cyber::SUCC;
}
...
int FusionCameraDetectionComponent::InitMotionService() {
  ...
  if (motion_buffer_ == nullptr) {
    motion_buffer_ = std::make_shared<base::MotionBuffer>(motion_buffer_size_);//jac!!20/1/14:create a circular buffer with a capacity for 100 MotionBuffer
  } else {
...
int FusionCameraDetectionComponent::InternalProc(
  ...
  prefused_message->process_stage_ = ProcessStage::MONOCULAR_CAMERA_DETECTION; //单目相机检测阶段
