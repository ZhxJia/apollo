...
bool ObstacleCameraPerception::Init(
  ...
  std::string config_file =
      GetAbsolutePath(options.root_dir, options.conf_file);//jac!!20/1/10:config file : "conf/perception/camera/obstacle.pt"
  config_file = GetAbsolutePath(work_root, config_file);
  CHECK(cyber::common::GetProtoFromFile(config_file, &perception_param_)) //jac!!20/1/10: 通过 config_file(obstacle.pt)中的参数部分修改 perception.proto中的参数
      << "Read config failed: ";
  ...
  // Init detector   jac!!20/1/9:/production/conf/camera/obstacle.pt
  ...
  for (int i = 0; i < perception_param_.detector_param_size(); ++i) {
    ...
    detector_init_options.root_dir =
        GetAbsolutePath(work_root, plugin_param.root_dir());// jac!!20/1/10: root_dir:../camera/models/yolo_obstacle_detector
    ...
    name_detector_map_.insert(
        std::pair<std::string, std::shared_ptr<BaseObstacleDetector>>(  //jac!!20/1/9: 相机名称与Detector的map
    ...
    CHECK(name_detector_map_.at(detector_param.camera_name())
              ->Init(detector_init_options))  //jac!!20/19: 初始化上述实例YoloObstacleDetector //yolo_obstacle_detector.cc/YoloObstacleDetector::Init()
        << "Failed to init: " << plugin_param.name();
  }//jac!!20/1/9: 上面这一部分 赋值相机参数 最后通过detector_init_options初始化了YoloObstacledetector实例(相关网络结构、参数) 并 初始化了（roipooling track）
  ...
  CHECK(perception_param_.has_tracker_param()) << "Failed to init tracker.";
  {
    ...
    tracker_init_options.root_dir =
        GetAbsolutePath(work_root, plugin_param.root_dir()); //jac!!20/1/11:omt_obstacle_tracker
    tracker_init_options.conf_file = plugin_param.config_file();//jac!!20/1/11:/config.pt
    tracker_.reset(
      ...
      << "Failed to init transformer.";
  {
    ...
    transformer_init_options.root_dir =
        GetAbsolutePath(work_root, plugin_param.root_dir());//jac!!20/1/11:multicue_obstacle_transformer
    transformer_init_options.conf_file = plugin_param.config_file();//jac??20/1/11:config.pt
    ...
    CHECK(transformer_->Init(transformer_init_options))
        << "Failed to init: " << plugin_param.name();//jac??20/1/11:最后初始化的object template 与 tracker初始化中的区别？
  }
  ...
  // Init feature_extractor  jac::externnal_feature
  if (!perception_param_.has_feature_param()) {
    AINFO << "No feature config found.";　//jac??20/1/11:obstacle.pt中并没有相关参数的初始化，因此执行该判断语句　特征提取器的初始化貌似是在detector部分执行的
    extractor_ = nullptr;
  } else {
    ...
    extractor_.reset(
        BaseFeatureExtractorRegisterer::GetInstanceByName(plugin_param.name()));//jac??20/1/11:obstacle.pt中并没有对此参数进行覆盖,不清楚其具体name是啥 可能执行了if 的第一条语句
  ...
  lane_calibration_working_sensor_name_ =
      options.lane_calibration_working_sensor_name;//jac!!20/1/11:"fusion_camera_detction_component.proto"->"front_6mm"

  // Init lane
  InitLane(work_root, model, perception_param_);//jac!!20/1/13 初始化lane_detector and lane_postprocesser
  ...
  // Init object template　jac!!20/1/13:包含了目标的长宽高速度等信息的模板
...
void ObstacleCameraPerception::InitLane(
  ...
  // Init lane
  CHECK(perception_param.has_lane_param()) << "Failed to include lane_param.";//jac!!20/1/13:perception_param:see perception.proto and obstacle.pt
  {
    ...
    lane_detector_.reset(BaseLaneDetectorRegisterer::GetInstanceByName(
        lane_detector_plugin_param.name()));//jac!!20/1/13:DarkSCNNLaneDetector
    CHECK(lane_detector_ != nullptr);
    CHECK(lane_detector_->Init(lane_detector_init_options)) //jac!!20/1/13:DarkSCNNLaneDetector 网络结构参数初始化
    ...
    postprocessor_init_options.detect_config_name =
        lane_detector_plugin_param.config_file();//config_darkSCNN.pt
    ...
    postprocessor_init_options.conf_file =
        lane_postprocessor_param.config_file();//lane_detector/config.pt
    lane_postprocessor_.reset(
        BaseLanePostprocessorRegisterer::GetInstanceByName(
            lane_postprocessor_param.name())); //jac!!20/1/13:DarkSCNNLanePostprocessor
...
void ObstacleCameraPerception::InitCalibrationService(
      ...
      << "Failed to include calibration_service_param.";
  {
    ...
    calibration_service_init_options.calibrator_working_sensor_name =
        lane_calibration_working_sensor_name_; //jac!!20/1/13:front_6mm
    calibration_service_init_options.name_intrinsic_map = name_intrinsic_map_;//jac!!20/1/13:相机内参和相机名称的map
    calibration_service_init_options.calibrator_method =
        calibration_service_param.calibrator_method();//jac!!20/1/13:lanelinecalibrator
...
bool ObstacleCameraPerception::Perception(
  ...
  if (lane_calibration_working_sensor_name_ ==
    ...
    PERCEPTION_PERF_BLOCK_END_WITH_INDICATOR(
        frame->data_provider->sensor_name(), "LaneDetector"); //jac!!20/2/6: 测试代码块运行时间
  ...
  // Obstacle prediction <output : proposed_objects>
