...
class ObjMapper {
 public:
  ObjMapper() : width_(0), height_(0) {
    ...
    resize_ry_score(params_.nr_bins_ry);//36
 ...
 private:
  ...
  float GetProjectionScore(float ry, const float *bbox, const float *hwl,
    ...
    float score = GetScoreViaRotDimensionCenter(
        ...
        bbox_res, bbox_near);//通过3D角点投影得到2d坐标对应的2dbbox与实际网络检测对应的2dbox的交并比 
    return score;
  }
  ...
  void PostRefineZ(const float *bbox, const float *hwl, const float *center2d,
    ...
    float z = center[2];//当前到物体的深度距离
    float score_best = 0.0f;
    float x[2] = {center2d[0], center2d[1]};
    float center_best[3] = {center[0], center[1], center[2]};
    for (float offset = -rz; offset <= rz; offset += dz) {  //(-1~1) 步长 0.2 m
      float z_test = std::max(z + offset, params_.depth_min);//depth_min = 0.1 ??
      float center_test[3] = {center[0], center[1], z_test};
      if (fix_proj) {
        common::IBackprojectCanonical(x, k_mat_, z_test, center_test);//通过反投影获得center_test=(X,Y,Z)
        center_test[1] += hwl[0] / 2;
      }
      float score_test = GetProjectionScore(ry, bbox, hwl, center_test);//改变深度之后计算此时投影得到的2dBox与网络检测的box交并比
  ...
  void FillRyScoreSingleBin(const float &ry) {
    ...
    int index = static_cast<int>(
        std::floor((ry + PI) * static_cast<float>(nr_bins_ry) / (2.0f * PI)));//std::floor 向下取整std::cell向上取整   (36/2pi)单位弧度对应的索引，乘以对应弧度可得索引,-pi为开始索引0的角度
    ry_score_[index % nr_bins_ry] = 1.0f; //将ry对应索引的分数置为1
  }
 ...
 private:
  ...
  int width_ = 0;
  int height_ = 0;
  std::vector<float> ry_score_ = {}; //size=36 (2pi 36等分)　网络检测2d框与3d角点投影的2d框的交并比 用于不确定度分析
