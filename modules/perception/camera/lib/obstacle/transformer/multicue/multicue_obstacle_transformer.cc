...
void MultiCueObstacleTransformer::SetObjMapperOptions(
  ...
  Eigen::Vector3f point_in_camera =
      static_cast<Eigen::Matrix<float, 3, 1, 0, 3, 1>>(
          camera_k_matrix.inverse() * image_point_low_center);//得到相机坐标系下的归一化平面底边中心点坐标
  *theta_ray =
      static_cast<float>(atan2(point_in_camera.x(), point_in_camera.z()));//得到底部中心点的相机角度。
  float rotation_y =
  ...
  obj_mapper_options->type_min_vol_index =
      MatchTemplates(sub_type, dimension_hwl); //当前检测到物体对应的模板尺寸
...
int MultiCueObstacleTransformer::MatchTemplates(base::ObjectSubType sub_type,
  ...
  const std::map<TemplateIndex, int> &kLookUpTableMinVolumeIndex =
      object_template_manager_->LookUpTableMinVolumeIndex(); //索引数　一般都是三
  ...
  switch (sub_type) {
    ...
    case base::ObjectSubType::MOTORCYCLIST: {
      ...
      float dh_mid = fabsf(dimension_hwl[0] - mid_tmplt_cur_type[0]); //高度的差值
...
void MultiCueObstacleTransformer::FillResults(
  ...
  obj->center = camera2world_pose * obj->center; //3d bbox中心转世界坐标系
  ...
  Eigen::Vector3d dir = (camera2world_pose.matrix().block(0, 0, 3, 3) *
                         Eigen::Vector3d(cos(theta), 0, -sin(theta)));//旋转角　转为方向向量
