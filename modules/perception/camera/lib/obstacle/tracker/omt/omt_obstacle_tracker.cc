...
bool OMTObstacleTracker::Init(const ObstacleTrackerInitOptions &options) {
  ...
  track_id_ = 0;
  frame_num_ = 0;
  frame_list_.Init(omt_param_.img_capability());//jac!!20/1/11:img_capabibity=14
  ...
  }//jac!!20/1/11:将type_change_cost中的数据存储到KTypeAssociateCost_中
  targets_.clear(); //清理vector中的元素

  used_.clear();
  // Init object template
  object_template_manager_ = ObjectTemplateManager::Instance(); //jac??20/1/11:与obstacle_reference中Init中最后一行的区别？ 实例化目标模板？？
  return true;
}
...
bool OMTObstacleTracker::Associate2D(const ObstacleTrackerOptions &options,
  ...
  for (size_t i = 0; i < frame->detected_objects.size(); ++i) { //循环此帧已检测目标
    ...
    TrackObjectPtr track_ptr(new TrackObject); //跟踪目标
    track_ptr->object = frame->detected_objects[i];
    track_ptr->object->id = static_cast<int>(i);//每一帧中检测物体的id
  ...
  for (auto &target : targets_) {
    if (target.lost_age > omt_param_.reserve_age()) { //5
