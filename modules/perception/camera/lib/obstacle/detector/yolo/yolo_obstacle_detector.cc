...
void YoloObstacleDetector::LoadParam(const yolo::YoloParam &yolo_param) {
  ...
  min_dims_.min_2d_height = model_param.min_2d_height(); //10
  min_dims_.min_3d_height = model_param.min_3d_height(); //0.1
  min_dims_.min_3d_width = model_param.min_3d_width();  
  min_dims_.min_3d_length = model_param.min_3d_length();
  ori_cycle_ = model_param.ori_cycle(); //jac??20/1/9:这个参数不太清楚
...
bool YoloObstacleDetector::InitNet(const yolo::YoloParam &yolo_param,
  ...
  const auto &model_param = yolo_param.model_param(); //jac!!20/1/10:注意Init()初始化了部分文件参数

  std::string proto_file =
      GetAbsolutePath(model_root, model_param.proto_file()); //jac!!20/1/10: deploy.pt
  std::string weight_file =
      GetAbsolutePath(model_root, model_param.weight_file()); //jac!!20/1/10: deploy.model
  ...
  AINFO << "model_type=" << model_type; //jac!!20/1/10:RTNetInt8
  inference_.reset(inference::CreateInferenceByName(model_type, proto_file,
                                                    weight_file, output_names,
                                                    input_names, model_root));//jac!!20/1/10:根据model_type创建Inference类实例inference_(例如RTNetInt8)
  ...
  if (!inference_->Init(shape_map)) { //jac??20/1/10:对应网络推断模型的初始化 应该有四类 但是具体实现 原理不明
    return false;
  }
  inference_->Infer();
  return true;
...
bool YoloObstacleDetector::Init(const ObstacleDetectorInitOptions &options) {
  ...
  base_camera_model_ = options.base_camera_model;   //jac!!20/1/9: 实例化该类 相机基础属性 宽，高，名称，（反）投影矩阵
  CHECK(base_camera_model_ != nullptr) << "base_camera_model is nullptr!";
  std::string config_path =
      GetAbsolutePath(options.root_dir, options.conf_file);
  if (!cyber::common::GetProtoFromFile(config_path, &yolo_param_)) { //jac??20/1/9: 改变proto中的部分参数
    AERROR << "read proto_config fail";
    return false;
  }//jac??20/1/9: 通过yolo_obstacle_detector/config.pt 赋值修改 yolo.proto部分参数?
  const auto &model_param = yolo_param_.model_param(); //jac!!20/1/9: 编译之后才存在
  std::string model_root =
      GetAbsolutePath(options.root_dir, model_param.model_name()); //jac!!20/1/9: /yolo_obstacle_detector/3d-r4-half/
  std::string anchors_file =
      GetAbsolutePath(model_root, model_param.anchors_file());  //jac!!20/1/9: anchors.txt 16类
  std::string types_file =
      GetAbsolutePath(model_root, model_param.types_file());    //jac!!20/1/9 :8类目标
  std::string expand_file =
      GetAbsolutePath(model_root, model_param.expand_file());   //jac??20/1/9: 这个文件应该与目标有关 具体不太清楚
  LoadInputShape(model_param); //jac!!20/1/9:对输入图像形状进行处理 jac??:具体实现效果不明
  LoadParam(yolo_param_); //jac!!20/1/9:load model_param and nms_param
  ...
  if (!LoadExpand(expand_file, &expands_)) {
    return false;
  }//jac??20/1/9:expand是扩充？
  CHECK(expands_.size() == types_.size());
  if (!InitNet(yolo_param_, model_root)) {
    return false;
  }//jac!!20/1/10:构建网络前向推断模型
  InitYoloBlob(yolo_param_.net_param());//jac??20/1/10:caffe blob 原理不明 所以这块没细看 待填坑
  if (!InitFeatureExtractor(model_root)) {
    return false;
  }//jac??20/1/10:这里貌似初始化了一个与检测相关的特征提取器用于跟踪(ROIpooling track) 
  return true;
}

bool YoloObstacleDetector::InitFeatureExtractor(const std::string &root_dir) {
  FeatureExtractorInitOptions feat_options;
  feat_options.conf_file = yolo_param_.model_param().feature_file(); //jac!!20/1/10:feature.pt: ROIpooling jac?? yolo中应该没用吧 貌似与跟踪有关
  ...
  if (!feature_extractor_->Init(feat_options)) {
    return false;
  }//jac!!20/1/10:Tracking_feat_extractor初始化 后面的plugin用到
  return true;
}
...
bool YoloObstacleDetector::Detect(const ObstacleDetectorOptions &options,
  ...
  DataProvider::ImageOptions image_options; //jac!!20/2/17 推断前图像的预处理
  ...
  frame->data_provider->GetImage(image_options, image_.get());//jac!!20/2/17 以Image8U为图像基本计算单元进行预处理操作
  ...
  /////////////////////////// detection part ///////////////////////////
  inference_->Infer();//jac!!20/2/18:网络模型前向推断
  ...
  filter_bbox(min_dims_, &(frame->detected_objects));//过滤框
  ...
  recover_bbox(frame->data_provider->src_width(),
               ...
               &frame->detected_objects);//将网络回归的bbox恢复到图像

  // post processing
  int left_boundary =
      static_cast<int>(border_ratio_ * static_cast<float>(image_->cols()));//border_ratio = 0.01
  int right_boundary = static_cast<int>((1.0f - border_ratio_) *
                                        static_cast<float>(image_->cols()));
  for (auto &obj : frame->detected_objects) {
    // recover alpha
    obj->camera_supplement.alpha /= ori_cycle_;// ori_cycle 1 
