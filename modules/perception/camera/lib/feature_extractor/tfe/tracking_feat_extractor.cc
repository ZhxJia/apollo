...
bool TrackingFeatureExtractor::Init(
  ...
  tracking_feature::FeatureParam feat_param; //jac!!20/1/10: tracking_feature.proto
  std::string config_path = cyber::common::GetAbsolutePath(
      init_options.root_dir, init_options.conf_file); //jac!!20/1/10:root_dir:3d-r4-half/feature.pt
  if (!cyber::common::GetProtoFromFile(config_path, &feat_param)) { //jac!!20/1/10:与之前用法类似，通过.pt文件中的部分参数覆盖proto中的参数
  ...
  if (feat_param.extractor_size() != 1) {
    ...
    return false;
  } //jac!!20/1/10: extractor 在proto中为repeated形式
...
void TrackingFeatureExtractor::init_roipooling(
  ...
  int pooled_w = param.pooled_w(); //3
  int pooled_h = param.pooled_h(); //3
...
bool TrackingFeatureExtractor::Extract(const FeatureExtractorOptions &options,
  ...
  if (!options.normalized) {
    encode_bbox(&(frame->detected_objects)); //相对于roi图像中心的坐标
  }
  for (auto feature_extractor_layer_ptr : roi_poolings_) {
    feature_extractor_layer_ptr->rois_blob->Reshape(
        {static_cast<int>(frame->detected_objects.size()), 5}); //bolb尺寸:(检测到的物体数,5)
    float *rois_data =
    ...
    for (const auto &obj : frame->detected_objects) {
      ...
      rois_data[2] =
          obj->camera_supplement.box.ymin * static_cast<float>(feat_height_); //feat_height_:feat_blob的高
      rois_data[3] =
          obj->camera_supplement.box.xmax * static_cast<float>(feat_width_); //feat_wdith_ : feat_blob的宽
      rois_data[4] =
      ...
      rois_data += feature_extractor_layer_ptr->rois_blob->offset(1); //新的物体,另一个rois_blob
    }
