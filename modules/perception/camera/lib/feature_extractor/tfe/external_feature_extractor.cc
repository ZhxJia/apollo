...
bool ExternalFeatureExtractor::Extract(const FeatureExtractorOptions &options,
  ...
  auto offset_y_ = static_cast<int>(
      param_.offset_ratio() * static_cast<float>(raw_height) + 0.5f);//0.288889*h+0.5
  image_options.crop_roi =
      base::RectI(0, offset_y_, raw_width, raw_height - offset_y_);//0,312,
  image_options.do_crop = true;
  // Timer timer;
  frame->data_provider->GetImage(image_options, image_.get());//get Image8U converted from raw message
