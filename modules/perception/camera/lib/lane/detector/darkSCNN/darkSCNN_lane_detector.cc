...
bool DarkSCNNLaneDetector::Init(const LaneDetectorInitOptions &options) {
  ...
  if (!GetProtoFromFile(proto_path, &darkscnn_param_)) {
    AINFO << "load proto param failed, root dir: " << options.root_dir;//jac!!20/1/13:obstacle.pt -> root_dir: /production/data/perception/camera/models/lane_detector/
    return false;
  }
  ...
  CHECK_LE(crop_height_, input_height_) //jac!!20/1/13:#define CHECK_LE(x,y) CHECK_OP(x,y,LE,<=)
  ...
  cudaDeviceProp prop;//jac!!20/1/13:GPU 先关参数和性能的数据结构　CUDA API
  cudaGetDeviceProperties(&prop, options.gpu_id);
  AINFO << "GPU: " << prop.name;

  const auto net_param = darkscnn_param_.net_param();
  net_inputs_.push_back(net_param.input_blob());//jac!!20/1/13:网络结构的输入输出节点
