...
定义参数通过DEFINE_type宏实现，如下所示，分别定义了一个bool和一个string类型的参数，
该宏的三个参数含义分别为命令行参数名，参数默认值，以及参数的帮助信息。*/
DEFINE_string(test_list, "full_test_list.txt", "test image list"); //front_6mm/test
...
bool LoadFromKitti(const std::string &kitti_path, CameraFrame *frame) {
  ...
  while (!feof(fp)) {
    ...
    obj->sub_type_probs.assign(
        static_cast<int>(base::ObjectSubType::MAX_OBJECT_TYPE), 0);//初始化为０
...
int main() {
  ...
  init_options.conf_file = FLAGS_detector_conf;//3d-r4-half-config.pt

  base::BrownCameraDistortionModel model; //相机畸变校正　Brown模型
  ...
  CHECK(transformer->Init(transformer_init_options));//multicue_obstacle_transformer  0 true
  AINFO << "Done!";
