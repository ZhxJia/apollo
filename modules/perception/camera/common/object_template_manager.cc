...
bool ObjectTemplateManager::Init(
  ...
  lib::MutexLock lock(&mutex_);//jac!!20/1/13:多线程互斥量mutex,保证代码独立运行
  if (inited_) {
    return true;
  }
  ...
  max_dim_change_ratio_ = proto.max_dim_change_ratio(); //最大尺寸变化率
...
float ObjectTemplateManager::Get3dDimensionSimilarity(const float *hwl1,
  ...
  float iou_h = min_h / (FLT_EPSILON + max_h);　//FLT_epsilon 为趋于0的常量
...
float ObjectTemplateManager::VehObjHwlBySearchTemplates(float *hwl, int *index,
  ...
  float hwl_flip[3] = {hwl[0], hwl[2], hwl[1]}; //翻转
  float score_best = -FLT_MAX;//浮点数的负最大值　用于保证比较
  int i_best = -1;
  int i3 = 0;
  bool from_flip = false;
  for (int i = 0; i < total_nr_tmplts_veh_; ++i) {　//
    float score = Get3dDimensionSimilarity(hwl, &veh_hwl_[i3]);
    float score_flip = Get3dDimensionSimilarity(hwl_flip, &veh_hwl_[i3]); //获取检测到的车辆尺寸与模板中尺寸的匹配程度　越接近于１越匹配
  ...
  }//如果是通过翻转长宽得到最高匹配分数，亦需要翻转长宽

  float dh = fabsf(hwl[0] - hwl_tmplt_matched[0]); //获取与最匹配模板的尺寸差值。
  ...
  float dh_dw_dl_ratio_mean = (dh_ratio + dw_ratio + dl_ratio) / 3;　//计算尺寸变化率的均值
  float dh_ratio_check = std::min(dh_ratio, dh_dw_dl_ratio_mean); //将高度变化率和均值变化率中的小值作为检测标准　
  if (score_best < FLT_EPSILON || dh_ratio_check > max_dim_change_ratio_) {　　//不能大于规定的最大尺寸变化率
    return -1.0f;
  }
  ...
  hwl[2] = veh_hwl_[i_best_by_3 + 2];　//将hwl赋值为最匹配的模板尺寸
  if (index != nullptr) {
    *index = i_best;
  }
  if (is_flip != nullptr) {
