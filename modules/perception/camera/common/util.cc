...
bool LoadAnchors(const std::string &path, std::vector<float> *anchors) {
  int num_anchors = 0;
  std::ifstream ifs(path, std::ifstream::in); //jac!!20/1/9:以只读方式打开
  ifs >> num_anchors; //jac!!20/1/9:获取第一个数据
  if (!ifs.good()) {
    AERROR << "Failed to get number of anchors!";
    return false;
  }
  (*anchors).resize(num_anchors * 2); //jac!!20/1/9:创建对象，改变容器大小
...
bool LoadTypes(const std::string &path,
  ...
  while (ifs >> type) {
    ...
    (*types).push_back(base::kName2SubTypeMap.at(type));//jac!!20/1/9 末尾插入
    AINFO << "\t\t" << type;
  }
