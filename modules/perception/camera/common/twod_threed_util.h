...
void GetBboxFromPts(const T *pts, const int &n, T *bbox) {
  ...
  for (int i = 1; i < n; ++i) {
    ...
    bbox[3] = std::max(bbox[3], y);//找n个角点映射的bbox2d的最大最小值
    i2 += 2;
  }
}
...
T GetScoreViaRotDimensionCenter(const T *k_mat, int width, int height,
  ...
  for (int i = 0; i < 8; ++i) {
    ...
    x_box[1] = y_corners[i];
    x_box[2] = z_corners[i];
    common::IProjectThroughKRt(k_mat, rot, location, x_box, x_proj);//获取角点的投影点
    x_proj[0] *= common::IRec(x_proj[2]);
    x_proj[1] *= common::IRec(x_proj[2]);//齐次坐标
  ...
  if (bbox_near != nullptr) {
    ...
    GetBboxFromPts(pts_proj, 4, bbox_front); //从4个的角点映射的2d点的最大最小值得到 bbox_front
    GetBboxFromPts(pts_proj + 8, 4, bbox_rear);//从８个..
    if (bbox_rear[3] - bbox_rear[1] > bbox_front[3] - bbox_front[1]) { //如何后面框的高大于前面框的高　由于近大远小　将对应的框给bbox_near
      memcpy(bbox_near, bbox_rear, sizeof(T) * 4);
    } else {
      memcpy(bbox_near, bbox_front, sizeof(T) * 4);
    }
  }
  return GetJaccardIndex(bbox, x_min, y_min, x_max, y_max);//获取bbox 之间的交并比
}
