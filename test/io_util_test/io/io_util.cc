...
//}
            ...
            bool GetFileList_(const std::string &path, const std::string &suffix,
                              std::vector<std::string> *files) {
                if (!PathExists(path)) {
                    AINFO << path << " not exist.";
                    return false;
...
using namespace apollo::perception::common;
...
EXPECT_TRUE(GetFileList("../conf", "config_manager.config",
                        &files));
}
...
int  main(int argc, char  *argv[]) {
    ...
    const std::string config_module_path = "../conf";
    std::vector<std::string> model_config_files;

    if (!GetFileList(config_module_path, "config_manager.config",
                      &model_config_files)) {
...
//    for (auto file :model_config_files) {
//        AINFO << "find file : " << file;
//    }
